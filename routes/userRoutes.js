...
module.exports = app => {
    ...
    app.get('/api/follow/:username', async (req, res) => {
        const isFollowing = await Follow.findOne({
            personFollowingId: req.user._id,
            personFollowedUsername: req.params.username
        });
        if (isFollowing) {
            res.send(true);
        } else {
    ...
    app.get('/api/followers/:id', async (req, res) => {
        const followers = await Follow.find({
            personFollowedId: req.params.id
        });
        res.send(followers);
    });
    ...
    app.get('/api/following/:id', async (req, res) => {
        const following = await Follow.find({
            personFollowingId: req.params.id
        });
        res.send(following);
    });
    ...
    app.patch('/api/user/:id', requireLogin, async (req, res) => {
        ...
        try {
            ...
            await Follow.updateMany(
                { personFollowingId: req.user.id },
                { $set: { personFollowingUsername: username } },
                { new: true }
            ).exec();
            await Follow.updateMany(
                { personFollowedId: req.user.id },
                { $set: { personFollowedUsername: username } },
                { new: true }
            ).exec();
    ...
    app.patch(
        ...
        async (req, res) => {
            ...
            try {
                ...
                const profile = await User.findByIdAndUpdate(
                    req.user.id,
                    { $set: { profileImage: data.Location } },
                    { new: true }
                ).exec();
                await Notification.updateMany(
                    { $and: [{ from: req.user.id }, { action: 'Follow' }] },
                ...
                await Follow.updateMany(
                    { personFollowingId: req.user.id },
                    { $set: { personFollowingImage: data.Location } },
                    { new: true }
                ).exec();
                await Follow.updateMany(
                    { personFollowedId: req.user.id },
                    { $set: { personFollowedImage: data.Location } },
                    { new: true }
                ).exec();
        ...
        if (follow === null) {
            ...
            try {
                ...
                await User.findOneAndUpdate(
                    { _id: personFollowingId },
                    { $inc: { followingCount: 1 } }
                ).exec();
                const user = await User.findOneAndUpdate(
                    { _id: personFollowedId },
